...
export default {
  ...
  mounted() {
    ...
    // var point = new BMapGL.Point(118.78, 32.07);
    var point = new BMapGL.Point(118.79166498333204, 32.074208655336658)
    ...
    // 添加图层
    const addLineLayer = () => {
      fetch('https://mapopen-pub-jsapigl.bj.bcebos.com/svgmodel/lineLayerData.json')
        .then(res => {
          return res.json()
        })
        .then(testLineData => {
      ...
      if (temporary && temporary.length > 1) {
        ...
        var polyline22 = new BMapGL.Polyline(
          [
            ...
            new BMapGL.Point(temporary[temporary.length - 1][0], temporary[temporary.length - 1][1])
          ],
